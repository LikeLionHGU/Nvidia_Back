...
public class GeminiApiClient {
    ...
    // 이미 가지고 계신 다른 분석 메서드들과 동일한 스타일
    public String generateChipsJson(String userText) {
        ...
        String raw = webClient.post()
                ...
                .map(GeminiApiClient::extractFirstText)
                .block();
    ...
    private static String extractFirstText(Map<?, ?> resp) {
        ...
        for (Object p : partsList) {
            if (p instanceof Map<?, ?> pm) {
                ...
                if (t instanceof String ts && !ts.isBlank()) return ts;
            }
        }
        return null;
    }
